...
export const VoicePoweredOrb: FC<VoicePoweredOrbProps> = ({
  ...
  useEffect(() => {
    ...
    const useExternal = typeof externalLevel === "number";
    ...
    try {
      ...
      if (enableVoiceControl && !useExternal) {
      ...
      const update = (t: number) => {
        ...
        let voiceLevel = 0;
        if (useExternal) {
          voiceLevel = Math.min(Math.max(externalLevelRef.current, 0), 1);
